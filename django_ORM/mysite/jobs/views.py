...
def past_life(request):
    ...
    persons = Person.objects.filter(name=names)
    """
    if persons==False:
        past_jobs = random.choice(job_past)
        Persons.objects.create(name=names,past_job=past_jobs)
        per
    ...
    """
    if Person.objects.filter(name=names):
        context = {
            'name' : names,
            'past_job' : Person.objects.filter(name=names)[0].past_job
        }
        return render(request,'jobs/past_life.html',context)
    else :
        past_jobs = random.choice(job_past)
        ...
        Person.objects.create(name=names,past_job=past_jobs)
        return render(request,'jobs/past_life.html',context)
